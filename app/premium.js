...
var Premium = function(){
	...
	var _dmx = [0,31,29,31,30,31,30,31,31,30,31,30,31];
	var _res = { code:0, left:0, limit:null };
	return {
		calc: function(s) {
			var R = 0, x = 0, y = 0;
			R = R ^ (-1);
			for (var i = 0, l = s.length; i < l; ++i) {
				y = (R ^ s.charCodeAt(i)) & 0xff;
				R = (R >>> 8) ^ tbl[y];
			}
			return (R ^ (-1)) >>> 0;
		},
		...
		validate: function(key) {
			...
			//    xxxxxxx  : user code of 6 figures number
			//    yyyymmdd : license limit date
			//    zzzzzzzz : hash value
			// ex: G123456-20151231-7FF40D34
			_res = { code:0, left:0, limit:null };
			var v = key.split('-');
			if (v.length == 3 && v[0].length == 7 && v[1].length == 8 && v[2].length == 8) {
				// check prefix
				if (v[0].charAt(0) != 'G') {
					_res.code = 2;
					return false;
				...
				// check date
				var y = parseInt(v[1].substr(0, 4), 10);
				var m = parseInt(v[1].substr(4, 2), 10);
				var d = parseInt(v[1].substr(6, 2), 10);
				if (y < 2015 || m < 1 || m > 12 || d < 1 || d > _dmx[m]) {
					_res.code = 3;
					return false;
				}
